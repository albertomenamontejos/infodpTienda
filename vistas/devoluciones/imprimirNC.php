...
<body>
	<div class="btn-group-vertical" id="botones">
		...
		<a id="btnHome"  href="<?php url("devoluciones/") ?>" class="btn btn-default"><span class="glyphicon glyphicon-home" aria-hidden="true" type="button"></span> Devoluciones</a>
	</div>	
...
<!--	</div>-->
	...
	<?php
		$comprobante = Comprobante::find($devolucion->idComprobante);
	?>
	<!-- nombre del comprobante, factura, nota de credito etc-->
	<div id="nombreComprobante"><?php echo $comprobante->descripcion ?></div>
	<div id="factSupDer">
		<label>Punto de Venta:</label><label>&nbsp;<?php printf("%06d",$parametro->puntoVenta);?></label>&emsp;&emsp;
		<label>Comp. Nro.</label><label><?php printf("%06d",$devolucion->nroNotaCredito); ?></label><br>
		<label>Fecha de Emisión:</label><label><?php echo date("d-m-Y",strtotime($devolucion->fecha));?></label><br>
		<label><?php echo $parametro->idenTributaria?>: </label><label><?php echo $parametro->cuit; ?></label><br>
...
<!--	<div id="factPeriodo">
	...
	<div id="factDatosCliente">
		<label><?php echo $parametro->idenTributaria?>:</label><label>&nbsp;<?php echo $cliente->cuit; ?></label><tab1></tab1>
		...
		$detallepagosventas = Detallepagoventa::where("idVenta",$devolucion->nroVenta);
	...
	<table id="tablaDetalle" class="table">
		...
		<tbody>
			...
			<tr>
				...
				<td>
					<?php 
						//busco el nombre del prod en tabla Productos
						$producto = Producto::find($detalledevolucion->idProducto);

					?>
					<?php echo trim($detalledevolucion->nomProd); ?>
				</td>
				<td><?php echo $detalledevolucion->cantDevuelta; ?></td>
				<td></td>		
				<td><?php echo sprintf('%0.2f',trim($detalledevolucion->precioUnitVenta)); ?></td>		
				<td></td>
				<td></td>			
				<!--calculo en tiempo real el subtotal neto-->
				<?php
					$subTotalDevuelto = $detalledevolucion->precioUnitVenta * $detalledevolucion->cantDevuelta;
				?>
				<td><?php echo sprintf('%0.2f',trim($subTotalDevuelto)); ?></td>
			</tr>			
			<?php
				} 
			?>			
	...
	<table id="tablaTotales" class="table">	
		<tbody>
			<tr>
				<td>Sub-Total: $</td>
				<td><?php echo sprintf('%0.2f',$subTotalDevuelto); ?></td>
			</tr>
			<tr>
